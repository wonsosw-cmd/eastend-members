...
`tools/local_config.json`(git 제외됨)에 웹앱 URL과 토큰을 넣고:

```
py -3.14 tools/export_members.py
```

→ `G:\공유 드라이브\...\★오프라인 회원관리\회원DB\회원목록_YYYYMMDD.xlsx` 생성.
작업 스케줄러에 등록하면 매일 자동 백업도 가능.
