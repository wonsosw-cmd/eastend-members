...
text(s, 0.9, 4.15, 11.0, 0.9,
     ...
     size=17, color=WHITE)
stats = [("신규가입 혜택", "3,000P 즉시 지급"), ("적립률", "구매액의 1% 즉시"), ("사용 (1P=1원)", "5,000P부터 1,000P 단위")]
...
title_bar(s, "OVERVIEW", "멤버십, 이렇게 돌아갑니다",
          "고객은 QR로 가입하고, 영수증으로 적립하고, 매장에서 포인트만큼 에누리 받습니다.")
flow = [
    ("1", "QR 스캔 → 가입", "매장 QR 포스터를 스캔해\n정보 입력·동의하면 가입 완료\n신규가입 3,000P 즉시 지급", TEAL),
    ("2", "영수증 적립 등록", "영수증 번호·금액을 등록하면\n결제금액의 1%가 즉시 적립\n(가입 익일부터 가능)", AMBER),
...
text(s, 1.0, 5.92, 11.4, 0.7,
     [[("매장에서 하는 일은 딱 3가지  —  ", {"bold": True, "color": NAVY}),
       ("① QR 가입 안내   ② 영수증 적립 등록   ③ 포인트 차감(에누리)", {"color": TEXT})]],
     size=15)
...
steps = [
    ...
    ("가입 완료", "신규가입 3,000P 즉시 지급 — 영수증 적립은 가입 다음 날부터"),
]
...
# ───────────────────────── 4. STEP 2 영수증 적립
s = slide()
title_bar(s, "STEP 2", "영수증 적립 — 두 가지 방법",
          "어느 쪽이든 결과는 같습니다. 결제금액의 1%가 등록 즉시 적립됩니다.")
box(s, 0.65, 1.95, 5.95, 3.0, fill=WHITE, line=LINE)
text(s, 1.0, 2.2, 5.2, 0.4, "방법 A  |  고객이 직접", size=16, color=TEAL, bold=True)
text(s, 1.0, 2.72, 5.3, 2.0,
     [["영수증 적립 페이지(receipt.html)에서"],
      ["전화번호 + 영수증 번호 + 결제금액 입력,"],
      ["영수증 사진 촬영(선택)"],
      [""],
      [("가입(회원)된 전화번호만 신청 가능", {"bold": True, "color": NAVY})]],
     size=13, color=TEXT, line_spacing=1.3)
box(s, 6.75, 1.95, 5.95, 3.0, fill=WHITE, line=LINE)
text(s, 7.1, 2.2, 5.2, 0.4, "방법 B  |  매장이 등록", size=16, color=AMBER, bold=True)
text(s, 7.1, 2.72, 5.3, 2.0,
     [["매장용 페이지에서 고객 전화번호 조회 후"],
      ["‘영수증 적립’ 탭에 번호 + 금액 입력"],
      [""],
      [("바쁘지 않다면 매장 등록을 권장", {"bold": True, "color": NAVY})],
      [("(입력 실수·누락이 적습니다)", {"color": MUTED, "size": 11.5})]],
     size=13, color=TEXT, line_spacing=1.3)
box(s, 0.65, 5.2, 12.05, 1.75, fill=NAVY_T)
text(s, 1.0, 5.4, 11.5, 0.35, "적립 규칙", size=14, color=NAVY, bold=True)
text(s, 1.0, 5.78, 11.5, 1.1,
     [[("·  적립액 = 결제금액의 1% (원단위 절사, 등록 즉시)      ·  같은 영수증 번호는 ", {}),
       ("1회만", {"bold": True, "color": RED}), ("  등록 가능", {})],
      [("·  영수증 적립은 ", {}), ("회원가입 익일부터", {"bold": True, "color": RED}),
       ("  가능 (가입 당일 불가 — 대신 웰컴 3,000P 즉시 지급)", {})],
      [("·  포인트는 ", {}), ("적립일로부터 1년", {"bold": True, "color": RED}),
       ("  경과 시 자동 소멸  ·  반품 시 본사로 알려주시면 차감 처리", {})]],
     size=13, color=TEXT, line_spacing=1.3)
...
# ───────────────────────── 6. STEP 4 조회·적립·차감
s = slide()
title_bar(s, "STEP 4", "일상 운영 — 조회 · 적립 · 차감",
          "모든 작업은 ‘전화번호 조회 → 고객 확인’ 후에 진행합니다.")
box(s, 0.65, 1.95, 3.9, 4.4, fill=WHITE, line=LINE)
text(s, 0.95, 2.2, 3.3, 0.4, "① 조회 & 본인 확인", size=15.5, color=NAVY, bold=True)
text(s, 0.95, 2.72, 3.35, 3.4,
     [["고객 전화번호 입력 → 조회"],
      [""],
      ["화면에 이름이 홍*동 처럼"],
      ["일부만 표시됩니다."],
...
box(s, 4.75, 1.95, 3.9, 4.4, fill=TEAL_T)
text(s, 5.05, 2.2, 3.3, 0.4, "② 영수증 적립", size=15.5, color=TEAL, bold=True)
text(s, 5.05, 2.72, 3.35, 3.4,
     [["‘영수증 적립’ 탭에서"],
      ["영수증 번호 + 결제금액 입력"],
      [""],
      [("→ 1% 즉시 적립", {"bold": True, "color": NAVY})],
      [("→ 바로 사용 가능 (5,000P부터)", {"bold": True, "color": NAVY})],
      [""],
      [("이미 등록된 영수증·가입 당일", {"color": MUTED, "size": 11.5})],
      [("고객은 자동으로 거부됩니다.", {"color": MUTED, "size": 11.5})]],
     size=12.5, color=TEXT, line_spacing=1.3)
box(s, 8.85, 1.95, 3.9, 4.4, fill=AMBER_T)
text(s, 9.15, 2.2, 3.3, 0.4, "③ 마일리지 차감 (에누리)", size=15.5, color=RGBColor(0xB0, 0x72, 0x1A), bold=True)
text(s, 9.15, 2.72, 3.35, 3.4,
     [["‘마일리지 차감’ 탭에서"],
      ["차감할 포인트 입력 (1P = 1원)"],
      [""],
      ...
      [""],
      [("잔액 초과·규칙 위반 시 차감되지", {"color": MUTED, "size": 11.5})],
      [("않고 안내 문구가 표시됩니다.", {"color": MUTED, "size": 11.5})]],
     size=12.5, color=TEXT, line_spacing=1.3)
...
faqs = [
    ("포인트는 언제 쌓이나요?", "영수증 등록 즉시 적립됩니다. 단, 영수증 적립은 회원가입 다음 날부터 가능합니다."),
    ("미가입 고객이에요", "QR 가입을 먼저 안내해주세요. 가입 즉시 웰컴 3,000P가 자동 지급됩니다."),
    ("영수증이 등록 안 돼요", "같은 영수증 번호는 한 번만 등록됩니다. 가입 당일 고객도 등록이 거부됩니다."),
